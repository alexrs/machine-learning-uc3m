...
class QLearningAgent(BustersAgent, ReinforcementAgent):
    ...
    def getValue(self, state):
        ...
        for action in self.getLegalActions(state):
            nextState[action] = self.getQValue(state, action)
        return nextState[nextState.argMax()]
    ...
    def computeQValueFromValues(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        print state
        return self.q_table[(state, action_s)]
    ...
    def update(self, state, action, nextState, reward):
        ...
        "*** YOUR CODE HERE ***"
        print state
        self.q_table[(state, action)] = (1-self.alpha)*self.q_table[(stateaction)] + self.alpha*(reward + self.discount*self.getValue(nextState))
        self.writeQtable()
